...
let rec handler buffer r w length =
  ...
  | `Eof ->
    ...
    (match Protocol.Parser.parse converted with
     ...
     | Error _ -> return (Writer.write w "Invalid command.\n"))
  | `Ok l -> handler buffer r w (length + l)
;;

let run () =
  let port = Tcp.Where_to_listen.of_port 8765 in
  (* TODO! Reduce buffer usage. *)
  let f _addr r w = handler (Bytes.create (4 * 1024)) r w 0 in
  ignore
